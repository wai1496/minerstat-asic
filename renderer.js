...
*/
  ...
  document.getElementById("button_addworker").addEventListener("click", function(e) {
      ...
      $.post("https://api.minerstat.com/api/asic_login.php", {}, function(data) {
          if (data == '1') { // Success
  ...
  function workersRefresh() {
      ...
      function loadWorkers(workersList) {
          ...
          for (var i = 0; i < splits.length; i++) {
              ...
              // Worker JSON
              wname = splits[i];
              wstatus = obj[wname]["status"];
              wcoin = obj[wname]["crypto"];
              wspeed = obj[wname]["5sec"];
              wunit = obj[wname]["5unit"];
              wuptime = obj[wname]["uptimec"];
              wdaily = obj[wname]["profit_usd_day"];
          ...
          var total_worker, total_online, total_daily;

          total_worker = obj["global"]["total"];
          total_online = obj["global"]["total_online"];
          total_daily = obj["global"]["totalprofit1"];
